...
void DrawSpriteVertex(int texNo, float X, float Y, float XMAX, float YMAX, float U, float V, float UW, float VH)
{
	D3D11_MAPPED_SUBRESOURCE msr;
	GetDeviceContext()->Map(g_VertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
	...
	// 左上を原点として設定するプログラム
	vertex[0].Position = D3DXVECTOR3(X, Y, 0.0f);
	vertex[0].Diffuse = D3DXCOLOR(0.8f, 0.8f, 0.6f, 1.0f);
	vertex[0].TexCoord = D3DXVECTOR2(U, V);

	vertex[1].Position = D3DXVECTOR3(XMAX, Y, 0.0f);
	vertex[1].Diffuse = D3DXCOLOR(0.8f, 0.8f, 0.6f, 1.0f);
	vertex[1].TexCoord = D3DXVECTOR2(U + UW, V);

	vertex[2].Position = D3DXVECTOR3(X, YMAX, 0.0f);
	vertex[2].Diffuse = D3DXCOLOR(0.8f, 0.8f, 0.6f, 1.0f);
	vertex[2].TexCoord = D3DXVECTOR2(U, V + VH);

	vertex[3].Position = D3DXVECTOR3(XMAX, YMAX, 0.0f);
	vertex[3].Diffuse = D3DXCOLOR(0.8f, 0.8f, 0.6f, 1.0f);
	vertex[3].TexCoord = D3DXVECTOR2(U + UW, V + VH);
